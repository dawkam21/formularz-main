...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $pdo->prepare("SELECT * FROM admins WHERE BINARY login = ? LIMIT 1");
    ...
    if ($user && password_verify($password, $user["password"])) {
        ...
        $_SESSION["role"] = $user["role"]; // Zapisujemy rolę do sesji
...
<body>
    ...
    <section id="form">
                <h1>Logowanie</h1>
            <form name="frmContact" method="post" action="admin-log.php">
                <div class="txt">
                    <label for="login">
